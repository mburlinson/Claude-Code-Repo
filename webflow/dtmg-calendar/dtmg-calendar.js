/**
 * Do The Most Good — Event Calendar
 * Webflow CMS Calendar Tool
 *
 * Cloned from rtc-calendar (lineage: rtc-calendar → dtmg-calendar).
 * RTC-specific featured-strip logic (fixFeaturedStripDates, computeNextOccurrence,
 * startOfTodayLocal, parseDateText) has been removed entirely — DTMG has no
 * featured-strip widget. Modal portal-to-body logic from explore-calendar retained.
 *
...
function buildRRule(startDate, frequency, dayOfWeek, weekOfMonth, endDate) {
  ...
  switch (frequency) {
    ...
    case 'Weekdays (Mon–Fri)':
    case 'Weekdays':
      // Mon–Fri only — for events that skip weekends
...
function showModal(info) {
  ...
  if (addCalBtn && addCalMenu) {
    ...
    var location = CALENDAR_CONFIG.location || 'Montgomery County, MD';
